...
from src.lib.siamese.dataset import generate_extraction_dataset, generate_validation_dataset
from src.lib.augmentations import *
from src.data.siamese_dataloader import ImageList, ContrastiveValList
from src.lib.siamese.model import SiameseNetwork
from lib.utils import imshow
from lib.io import *



def generate_features(args, net, query_images, data_loader):
    features_list = list()
    images_list = list()
    t0 = time.time()
    with torch.no_grad():
    ...
    write_hdf5_descriptors(features, images, args.query_f)
    print(f"writing descriptors to {args.db_f}")
    print(f"image_description_time: {(t1 - t0) / len(query_images):.5f} s per image")
    return features


def extract_features(siamese_args, visualization=False):
    # TODO: Returning the ground truth labels for a given dataset
    groundtruth_list = read_ground_truth(siamese_args.gt_list)
    query_list = [l.strip() for l in open(siamese_args.query_list, "r")]
    database_list = [l.strip() for l in open(siamese_args.db_list, "r")]
    train_list = [l.strip() for l in open(siamese_args.train_list, "r")]

    # creating the dataset
    query_images, database_images, train_images = generate_extraction_dataset(query_list, database_list, train_list)
    ...
    query_features = generate_features(args, net, query_images, query_loader)
    database_features = generate_features(args, net, database_images, db_loader)
